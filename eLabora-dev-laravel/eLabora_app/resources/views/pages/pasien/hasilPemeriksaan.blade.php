...
<div class="max-w-7xl mx-auto space-y-5">
  ...
  <div>
    <h1 class="text-2xl font-bold text-gray-900">Hasil Pemeriksaan</h1>
    <p class="text-sm text-gray-500">
      Lihat hasil pemeriksaan berdasarkan kategori dan nomor lab.
    </p>
  </div>
  ...
  <form method="GET" action="{{ route('hasil.pemeriksaan') }}">
  ...
  <div class="flex flex-wrap gap-2">
    @foreach ($tabs as $key => $label)
      <a href="{{ route('hasil.pemeriksaan', ['kategori' => $key, 'q' => $qAktif]) }}" class="text-sm font-medium px-5 py-2.5 rounded-lg {{ $kategoriAktif === $key ? 'text-white bg-indigo-600' : 'text-gray-900 bg-white border border-gray-300 hover:bg-gray-100' }}">
        {{ $label }}
      </a>
    @endforeach
  </div>
  ...
  @if (empty($items) || count($items) === 0)
    <div class="flex items-center justify-center p-10 bg-white border-2 border-gray-300 border-dashed rounded-lg">
      <p class="text-sm text-gray-500">Tidak ada hasil yang sesuai.</p>
    </div>
  @else
    <div class="space-y-4">
      @foreach ($items as $it)
        @php
          $pemeriksaanId = $it['pemeriksaan_id'] ?? ($it['id'] ?? null);
        ...
        {{-- Flowbite Card --}}
        <a href="{{ route('detail.pemeriksaan', $pemeriksaanId) }}" class="block p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 transition-all hover:shadow-lg">
