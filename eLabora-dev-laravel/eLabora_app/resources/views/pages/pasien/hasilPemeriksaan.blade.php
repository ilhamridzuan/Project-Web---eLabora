...
<div class="max-w-4xl space-y-5">
  ...
  <form method="GET" action="{{ route('hasil.pemeriksaan') }}" class="space-y-3">
    ...
    <div class="relative">
      <input
        ...
        class="w-full px-4 py-3 border border-[#dce1f2] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white pr-24" />
      <button
        type="submit"
  ...
  <div class="flex flex-wrap gap-3">
    @foreach ($tabs as $key => $label)
      <a
        href="{{ route('hasil.pemeriksaan', ['kategori' => $key, 'q' => $qAktif]) }}"
  ...
  @else
    <div class="space-y-4">
      @foreach ($items as $it)
        ...
        {{-- Klik card -> detail pemeriksaan --}}
        <a href="{{ route('detail.pemeriksaan', $pemeriksaanId) }}" class="block">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:gap-4">
              ...
              <div class="flex h-12 w-12 items-center justify-center rounded-2xl ring-1 {{ $iconClass($kategoriNama) }}">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  <path
                    d="M9 3v5l-5 9a4 4 0 0 0 3.5 6h9a4 4 0 0 0 3.5-6l-5-9V3"
                    stroke="currentColor"
                    stroke-width="2"
