<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 lg:translate-x-0" aria-label="Sidebar">
    <div class="h-full px-3 pb-4 overflow-y-auto bg-white flex flex-col">
        <ul class="space-y-2 font-medium flex-1">
            @php
            $items = [
                ['label' => 'Beranda', 'url' => url('/dashboard-pasien'), 'icon' => 'icon-[tabler--home]'],
                ['label' => 'Pendaftaran', 'url' => url('/pendaftaran'), 'icon' => 'icon-[tabler--calendar-plus]'],
                ['label' => 'Cek Hasil Pemeriksaan', 'url' => url('/hasil-pemeriksaan'), 'icon' => 'icon-[tabler--clipboard-text]'],
                ['label' => 'Riwayat Pemeriksaan', 'url' => url('/riwayat'), 'icon' => 'icon-[tabler--history]'],
            ];
